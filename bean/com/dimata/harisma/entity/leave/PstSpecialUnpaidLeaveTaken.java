...
/**
    ...
    public static float getSpUnQty(long employeeId, Date startDate, Date endDate, long[] scheduleIds) {
        ...
        String where = fieldNames[FLD_SCHEDULED_ID] + " IN "+ inScheId + " AND  (SU." + 
                fieldNames[FLD_EMPLOYEE_ID] + "=\""+ employeeId + "\") AND  ((SU." + fieldNames[FLD_TAKEN_DATE] + " BETWEEN \""
                + Formater.formatDate(startDate, "yyyy-MM-dd ") + "00:00:00\" AND \""
                + Formater.formatDate(endDate, "yyyy-MM-dd ") + "23:59:59\") OR "
                + " (SU." + fieldNames[FLD_TAKEN_FINNISH_DATE] + " BETWEEN \""
                + Formater.formatDate(endDate, "yyyy-MM-dd ") + "00:00:00\" AND \""
                + Formater.formatDate(endDate, "yyyy-MM-dd ") + "23:59:59\")) AND LA."+PstLeaveApplication.fieldNames[PstLeaveApplication.FLD_DOC_STATUS]+" IN (2,3)";

        Vector list = listJointWithLeaveApplication(0, 100, where, fieldNames[FLD_TAKEN_DATE]);
