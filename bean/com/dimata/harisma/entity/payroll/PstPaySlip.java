...
//            formSQL = formSQL.substring(1);
              formSQL = formSQL.substring(1).trim();
        }
        String tmpMessage = "";
//        double compFormValue = PstPaySlipComp.getCompFormValue(formSQL, tmpMessage);
            double compFormValue = 0.0; // add by Kartika 2022-10-14
            boolean doExecuteFormSQL= true;  // add by Kartika 2022-10-14
            try{// add by Kartika 2022-10-14
              compFormValue = Double.parseDouble(formSQL);
              doExecuteFormSQL = false;
            }catch(Exception excDb){
                if(isDebugModePersistent()){
                    System.out.println(excDb);
                }
            }
            if(doExecuteFormSQL) // add by Kartika 2022-10-14 execute value if only formSQL is a formula not 0 or 0.0 value 
                 compFormValue = PstPaySlipComp.getCompFormValue(formSQL, tmpMessage);
