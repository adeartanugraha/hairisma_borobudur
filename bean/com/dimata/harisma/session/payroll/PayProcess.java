...
//        listEmpLevel = PstPayEmpLevel.listEmpLevelwithDateResign(levelCode, getLevelSel(), oidDivision, oidDepartment, minCommencing, PstEmployee.NO_RESIGN, payPeriod.getStartDate(), payPeriod.getEndDate(), rsgnStartdate, rsgnEnddate, payrollGroupId, empNum);
        ...
        this.setRunning(true);
        {
            try {
                ...
                // x
                long tempDurasiMs = 0; // add by Kartika 2022-10-17
                long tempDurasiCompMs = 0; // add by Kartika 2022-10-17
                Hashtable<String, Vector> hashLevelComponent = new Hashtable();// add by Kartika 2022-10-17
                Hashtable<String, PayComponent>hashPayComponent = new Hashtable();// add by kartika 2022-11-04
                recordSize = listEmpLevel!=null ? listEmpLevel.size():0;
                for (int p = 0; p < listEmpLevel.size(); p++) {
                    tempDurasiMs = System.currentTimeMillis();
                    ...
                    try {
                        // *** add & update by Kartika 2022-10-17 FROM  ***
                        String levelGroupCode = "" + payEmpLevel.getLevelCode() + (this.payGroupId != 0 ? (" AND " + PstPayComponent.fieldNames[PstPayComponent.FLD_PAYSLIP_GROUP_ID] + "=" + this.payGroupId) : "");
                        Vector component = hashLevelComponent.get(levelGroupCode);
                        if (component == null) {
...
//                             " AND " + PstSalaryLevelDetail.fieldNames[PstSalaryLevelDetail.FLD_FORMULA] + " NOT LIKE '%IN\\_%'" + (this.payGroupId!=0 ?(" AND "+PstPayComponent.fieldNames[PstPayComponent.FLD_PAYSLIP_GROUP_ID]+"="+this.payGroupId):"" ), empId);
                    ...
                    /* String where = PstSalaryLevelDetail.fieldNames[PstSalaryLevelDetail.FLD_LEVEL_CODE] + "='" + payEmpLevel.getLevelCode() + "'" +
                        ...
                        hashLevelComponent.put(levelGroupCode, component);
                        }
...
//                                    PayConfigPotongan configPot = (PayConfigPotongan) payConfigList.get(i);
                              if (payConfigList != null && payConfigList.size() > 0) {
                                PayConfigPotongan configPot = (PayConfigPotongan) payConfigList.get(0);
                                        try {
                                            PayComponent payComp = hashPayComponent.get(""+configPot.getComponentId()); // add by kartika 2022-11-04
                                            if(payComp==null){        // add by kartika 2022-11-04
                                                payComp = PstPayComponent.fetchExc(configPot.getComponentId());
                                                hashPayComponent.put(""+configPot.getComponentId(), payComp);
                                            }
...
//                                    }
                        ...
                        for (int c = 0; c < component.size(); c++) {
                            try {
                                ...
                                tempDurasiCompMs = System.currentTimeMillis(); // 2022-10-31
                           ...
                           //         EmpSchedule empSch = PstEmpSchedule.getEmpSchedule(empId, period.getOID(), diffDay, dtPeriodNew);
                                ...
                                if (p == 0 && DBHandler.isDebugModeController()) {
                                    summaryDurationComp = summaryDurationComp + "<br>" + compCode + ": " + (System.currentTimeMillis() - tempDurasiCompMs + " ms");
                                }
                    ...
                    totalDuration += (System.currentTimeMillis() - tempDurasiMs);
                    setSumMessage(sumMessage + " durasi: " + (System.currentTimeMillis() - tempDurasiMs + " ms"));   // added by Kartika 2022-10-17
                }
            } catch (Exception exc) {
                System.out.println(exc);
        ...
        this.running = false;
        message = "Payroll calculation done " + "<br> Duration Total = " + totalDuration + " ms<br> Duration per component: " + summaryDurationComp;
    }
