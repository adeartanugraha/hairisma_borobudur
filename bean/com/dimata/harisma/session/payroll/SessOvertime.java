...
//                        }
    ...
    public static String calcOvTmDayOff(OvertimeDetail overtime, boolean reloadMap, float minOvertimeHour) {
        ...
        if (reloadMap || (!levelMalLoaded)) {
            dPLevelToOvTypeHoliday = PstOvt_Type.getLevelOvIdxMap(PstOvt_Type.HOLIDAY);
            dPLevelToOvTypeWork = PstOvt_Type.getLevelOvIdxMap(PstOvt_Type.WORKING_DAY);
            dPLevelToOvTypeHolidayMasterLevel = PstOvt_Type.getLevelOvMapLevelOID(PstOvt_Type.HOLIDAY);
            dPLevelToOvTypeWorkMasterLevel = PstOvt_Type.getLevelOvMapLevelOID(PstOvt_Type.WORKING_DAY);        }
        try {
            ScheduleSymbol sch = PstEmpSchedule.getDailySchedule(overtime.getDateFrom(), overtime.getEmployeeId());
