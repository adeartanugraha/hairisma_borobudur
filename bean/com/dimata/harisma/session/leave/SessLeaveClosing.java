...
//update by satrya 2012-11-08
    ...
    //ADD BY ERI YUDI 20200909 for check emp not yet have ll stock management;
    public static Vector getEmployeeNotYetHaveStockLLandEnttile(I_Leave leaveConfig) {
        ...
        try {
            ...
            String sql = "SELECT EMP."
                    ...
                    + PstEmpCategory.fieldNames[PstEmpCategory.FLD_EMP_CATEGORY]
                    + " FROM " + PstEmployee.TBL_HR_EMPLOYEE + " EMP LEFT JOIN "
                    + PstLLStockManagement.TBL_LL_STOCK_MANAGEMENT + " LLM ON LLM."
                    ...
                    + PstLevel.fieldNames[PstLevel.FLD_LEVEL_ID] + " INNER JOIN " + PstEmpCategory.TBL_HR_EMP_CATEGORY + " CAT ON EMP."
                    + PstEmployee.fieldNames[PstEmployee.FLD_EMP_CATEGORY_ID] + "= CAT." + PstEmpCategory.fieldNames[PstEmpCategory.FLD_EMP_CATEGORY_ID]
                    + " WHERE ( LLM." + PstLLStockManagement.fieldNames[PstLLStockManagement.FLD_LL_STOCK_ID] + " is null OR LLM."
                    + PstLLStockManagement.fieldNames[PstLLStockManagement.FLD_LL_STOCK_ID] + " = 0 ) AND EMP."
                    + PstEmployee.fieldNames[PstEmployee.FLD_RESIGNED] + " = " + PstEmployee.NO_RESIGN 
                    + " AND CAT."+ PstEmpCategory.fieldNames[PstEmpCategory.FLD_ENTITLE_FOR_LEAVE] + " = " + PstEmpCategory.ENTITLE_YES
                    + " AND ( EMP."
                    + PstEmployee.fieldNames[PstEmployee.FLD_COMMENCING_DATE] + " != '0000-00-00' OR EMP."
                    + PstEmployee.fieldNames[PstEmployee.FLD_COMMENCING_DATE] + " != null ) "
                    + " AND ( (DATE_ADD(EMP." + PstEmployee.fieldNames[PstEmployee.FLD_COMMENCING_DATE] + ", INTERVAL "+leaveConfig.getIntervalLLinMonths()[LeaveConfigHR.iINTERVAL_LL_HARDROCK_I]+" MONTH ) <= CURRENT_DATE) OR "
                    + "(DATE_ADD(EMP." + PstEmployee.fieldNames[PstEmployee.FLD_COMMENCING_DATE] + ", INTERVAL "+leaveConfig.getIntervalLLinMonths()[LeaveConfigHR.iINTERVAL_LL_HARDROCK_II]+" MONTH ) <= CURRENT_DATE) )"
                    + " ORDER BY EMP."
            ...
            while (rs.next()) {
//                int LoS = (int) DateCalc.dayDifference( rs.getDate(PstEmployee.fieldNames[PstEmployee.FLD_COMMENCING_DATE]), new Date());
//                float entitled = leaveConfig.getLLEntile(rs.getString(PstLevel.fieldNames[PstLevel.FLD_LEVEL]), 
//                        //int entitled = leaveConfig.getALEntitleAnualLeave(rs.getString(PstLevel.fieldNames[PstLevel.FLD_LEVEL]), 
//                        rs.getString(PstEmpCategory.fieldNames[PstEmpCategory.FLD_EMP_CATEGORY]), LoS);

 //               if (entitled > 0) {

                    LeaveLlClosingList leaveLLClosingNoStockList = new LeaveLlClosingList();

                    leaveLLClosingNoStockList.setEmpId(rs.getLong(PstEmployee.fieldNames[PstEmployee.FLD_EMPLOYEE_ID]));
                    leaveLLClosingNoStockList.setEmpNum(rs.getString(PstEmployee.fieldNames[PstEmployee.FLD_EMPLOYEE_NUM]));
                    leaveLLClosingNoStockList.setFullName(rs.getString(PstEmployee.fieldNames[PstEmployee.FLD_FULL_NAME]));
                    leaveLLClosingNoStockList.setCommancingDate(rs.getDate(PstEmployee.fieldNames[PstEmployee.FLD_COMMENCING_DATE]));
                    leaveLLClosingNoStockList.setDepartmentId(rs.getLong(PstEmployee.fieldNames[PstEmployee.FLD_DEPARTMENT_ID]));
                    leaveLLClosingNoStockList.setDepartment(rs.getString(PstDepartment.fieldNames[PstDepartment.FLD_DEPARTMENT]));
                    leaveLLClosingNoStockList.setExpiredDate(rs.getDate(PstEmployee.fieldNames[PstEmployee.FLD_COMMENCING_DATE]));
                    leaveLLClosingNoStockList.setEntitledDate(rs.getDate(PstEmployee.fieldNames[PstEmployee.FLD_COMMENCING_DATE]));
                    leaveLLClosingNoStockList.setEntitled(0);
                    result.add(leaveLLClosingNoStockList);
//                }
            }
            rs.close();
            return result;
...
//update by satrya 2010-10-16
    ...
    public static void insertStockEmpty(long employee_id) {
        ...
        Date empDate1years = new Date(employee.getCommencingDate().getTime());
        empDate1years.setYear(empDate1years.getYear()+1);
...
//                leaveLlClosingList.setCommancingDate(newComm);
    ...
    public static Vector listLLClosingALL() {
        ...
        try {
            ...
            String sql = "SELECT LLM." + PstLLStockManagement.fieldNames[PstLLStockManagement.FLD_LL_STOCK_ID] + ",LLM."
                    ...
                    +" ( LLM." + PstLLStockManagement.fieldNames[PstLLStockManagement.FLD_LL_STATUS] + "=" + PstLLStockManagement.LL_STS_AKTIF + " AND "
                    + PstLLStockManagement.fieldNames[PstLLStockManagement.FLD_ENTITLED_DATE] + " < CURRENT_DATE AND "
                    + " DATE_ADD(LLM." + PstLLStockManagement.fieldNames[PstLLStockManagement.FLD_ENTITLED_DATE] + ", INTERVAL "
    ...
    public static void ProcessClosingLL(Vector ll_closing_selected) {
        ...
        for (int i = 0; i < ll_closing_selected.size(); i++) {
            ...
            }else if  (llClosingSelected.getStatusData() == LL_AKTIF) {
                processNoStockLLManagement(llClosingSelected);
            }
        }
    }
    ...
    //add by Eri Yudi 20200909 
        //untuk memproses yang belum punya ll stock management
    public static void processNoStockLLManagement(LlClosingSelected llClosingSelected) {
            LLStockManagement llStockManagementData = null;
                     if(llClosingSelected !=  null && llClosingSelected.getEmployeeId() != 0){
                        Vector listLlStockMan = (Vector) PstLLStockManagement.list(0, 0, " "+PstLLStockManagement.fieldNames[PstLLStockManagement.FLD_EMPLOYEE_ID]+" = "+llClosingSelected.getEmployeeId(), " "+PstLLStockManagement.fieldNames[PstLLStockManagement.FLD_LL_STOCK_ID]+" ");
                        if(listLlStockMan.size() > 0){
                            for(int x = 0 ; x < listLlStockMan.size();x++ ){
                            llStockManagementData = (LLStockManagement) listLlStockMan.get(1);
                            }
                        }else{
                            llStockManagementData = new LLStockManagement(); 
                        }
                    }
                    long llStockManagementId =0;
                    
                    if( llStockManagementData.getOID() == 0){                                             
                        llStockManagementData.setEmployeeId(llClosingSelected.getEmployeeId());
                        llStockManagementData.setLLStatus(PstLLStockManagement.LL_STS_AKTIF);
                        llStockManagementData.setLeavePeriodeId(0);                        
                        llStockManagementData.setEntitled(0);
                        llStockManagementData.setQtyUsed(0);
                        llStockManagementData.setOpeningLL(0);
                        llStockManagementData.setPrevBalance(0);                                                                                              
                        llStockManagementData.setLLQty(0);
                        llStockManagementData.setQtyResidue(0);
                        llStockManagementData.setRecordDate(new Date());                                                 
                        llStockManagementData.setEntitledDate(llClosingSelected.getCommencingDate());                        
                        llStockManagementData.setExpiredDate(llClosingSelected.getCommencingDate());
    ...
    public static void processClosingEntitle2(long stock_id) {
        ...
        //add By Eri Yudi 2020-10-16
        float qtyUsed2 = llStockManagement.getQtyResidue() + llStockManagement.getQtyUsed();
        float qtyResidu2 = 0;
        //end add By Eri
        LlStockExpired llStockExpired = new LlStockExpired();
        ...
        try {
            String sql = "UPDATE " + PstLLStockManagement.TBL_LL_STOCK_MANAGEMENT + " SET "
                    ...
                    //add by Eri Yudi 2020-10-15
                     + PstLLStockManagement.fieldNames[PstLLStockManagement.FLD_QTY_USED] + "='" + qtyUsed2 + "',"
                    + PstLLStockManagement.fieldNames[PstLLStockManagement.FLD_QTY_RESIDUE] + "='" +qtyResidu2+ "',"
                    //end add by Eri
...
//                        + " = " + "\"" + empNumId.trim() + "\"";//penambahan trima
    ...
    public static Vector listEmpEligibleLeave(){
        Vector result = new Vector();
        DBResultSet dbrs = null;
        try {
            String sql = "SELECT emp.* FROM "+PstEmployee.TBL_HR_EMPLOYEE+" emp " +
                    " INNER JOIN "+PstEmpCategory.TBL_HR_EMP_CATEGORY+" cat " +
                    ...
                    " WHERE cat." + PstEmpCategory.fieldNames[PstEmpCategory.FLD_ENTITLE_FOR_LEAVE] +
                    " = 1 AND emp." + PstEmployee.fieldNames[PstEmployee.FLD_RESIGNED]+" = 0";
