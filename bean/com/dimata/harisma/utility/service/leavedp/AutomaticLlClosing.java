...
public class AutomaticLlClosing implements Runnable {
    ...
    public void llClosing(I_Leave leaveConfig) {
        ...
        Vector resultll_no_stock = SessLeaveClosing.getEmployeeNotYetHaveStockLLandEnttile(leaveConfig);
        Vector result_close = new Vector();
        
          // add by Eri Yudi 20200909 untuk generate ll stock management jika belum punya
        for(int x = 0; x < resultll_no_stock.size() ; x++){
             LeaveLlClosingList leaveLlClosingList = (LeaveLlClosingList)resultll_no_stock.get(x);
        ...
        int max_size=0;
        int max_iterasi=0;
        if(resultLL_all !=  null){
         max_size = resultLL_all.size();
         max_iterasi = resultLL_all.size() -1 ;
        }
        int max_count = 0;
        int i = 0 ;
