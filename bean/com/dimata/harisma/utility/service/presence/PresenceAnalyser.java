...
//                        }
                           ...
                           //priska fungsi delete dp 20160517
                        long fungsiDP = fungsiDPRamaResto(selectedDate, employeeId, scheduleCategory);
...
//                        }
    ...
    // add by eri yudi 20200904 untuk analisa dp
        public static long fungsiDPRamaResto(Date selectedDate, long employeeId, int scheduleCategory){
        long nilai = 0;
        ...
        //priska fungsi delete dp 20160517
                        boolean adaDp = false;
                        boolean adaPublicHolidays = false;
                        
                        DpStockManagement dpStockManagement = new DpStockManagement();
                        try{
                        dpStockManagement =  PstDpStockManagement.getDpStockManagement(selectedDate, employeeId);
                        } catch (Exception e){}
                        
                        if (dpStockManagement != null && dpStockManagement.getOID() !=0){
                            adaDp = true;
                        }
                        
                        PublicHolidays publicHolidays = new PublicHolidays();
                        try { 
                          publicHolidays = PstPublicHolidays.getPublicHolidaysByDate(selectedDate, employeeId);
                        } catch (Exception e){}
                        if (dpStockManagement != null && dpStockManagement.getOID() !=0){
                            adaDp = true;
                        }
                        
                        if (adaDp && publicHolidays != null && publicHolidays.getOID() != 0 ){
                            try{
                                long delDP = PstDpStockManagement.deleteExc(dpStockManagement.getOID());
                              adaDp = false;
                            }catch(Exception e){}
                        ...
                        //add by eri 20200904 untuk hbj
                         Level level = new Level();
                        try { 
                            Employee emp = (Employee)PstEmployee.fetchExc(employeeId);
                              level = (Level) PstLevel.fetchExc(emp.getLevelId());
                        } catch (Exception e){
                            level.setEntitleDP(0);
                            level.setEntitledDPQty(0);
                        }
                        int date = selectedDate.getDate();
                        Period period = (Period) PstPeriod.getPeriodBySelectedDate(selectedDate);
                        int presenceStatus = PstEmpSchedule.getStatus1(date, employeeId, selectedDate);
                        
                       
                        boolean hadir = false;
                        if(presenceStatus != PstEmpSchedule.STATUS_PRESENCE_ABSENCE && presenceStatus != PstEmpSchedule.STATUS_PRESENCE_NOT_PROCESSED){
                            hadir = true;
                        }
                        
                        
                        if (!adaDp && (scheduleCategory != PstScheduleCategory.CATEGORY_OFF) && hadir &&  level.getEntitleDP()!=0 && publicHolidays != null && publicHolidays.getOID() != 0 ) {
                            
                            
                            Date expDate = null;
                            expDate = (Date) selectedDate.clone();
                            ...
                            DpStockManagement dpStockManagement1 = new DpStockManagement();
                            dpStockManagement1.setQtyResidue(level.getEntitledDPQty());

                            dpStockManagement1.setiDpQty(level.getEntitledDPQty());
                            dpStockManagement1.setDtOwningDate(selectedDate);
                            dpStockManagement1.setDtExpiredDate(expDate);
                            dpStockManagement1.setiDpStatus(0);
                            dpStockManagement1.setEmployeeId(employeeId);
                            dpStockManagement1.setStNote(" DP Generate " + selectedDate);
                            DpStockManagement dpStockManagementX = new DpStockManagement();
                            try {
                                dpStockManagementX = PstDpStockManagement.getDpStockManagement(selectedDate, employeeId);
                            } catch (Exception e) {
                            }
                            if (dpStockManagementX.getOID() == 0) {
                                try {
                                    long oid = PstDpStockManagement.insertExc(dpStockManagement1);
                                } catch (Exception x) {
                                }
