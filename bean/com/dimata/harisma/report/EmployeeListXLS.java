...
//        srcEmployee = (SrcEmployee) session.getValue(SessEmployee.SESS_SRC_EMPLOYEE);
        ...
        String[] tableHeader = {
            ...
            "NO","PAYROLL","BARCODE_NUMBER","ID CARD","FULL_NAME","COMPANY","DIVISION","DEPARTEMENT","SECTION","EMP. CATEGORY","Payroll Group"
            ,"LEVEL","POSITION","EMAIL","RELIGION","GENDER","EDUCATION","GRADUATION","START_DATE_EDUCATION","END_DATE_EDUCATION"
            ,"LANGUAGE","ORAL","WRITTEN","DESCRIPTION","MARITAL STATUS","TAX REPORT","BLOOD"
            ,"BPJS KETENAGAKERJAAN NUMBER","BPJS KETENAGAKERJAAN DATE","COMMENCING DATE","LoS","RESIGN DATE","RESIGN REASON","RESIGN DESC"
        ...
        for(int c=0; c<listCustom.size(); c++){
            ...
            cell = row.createCell((short) cntH);   
            cell.setCellValue(custom.getFieldName());
            cell.setCellStyle(style3);
            cntH++;
        }
     ...
     if(listEmployee!=null&&listEmployee.size()>0){
         SessTmpSpecialEmployee sessTmpSpecialEmployee = new SessTmpSpecialEmployee();
          Vector listEducation = PstEmpEducation.listEducation("", PstEmpEducation.fieldNames[PstEmpEducation.FLD_EMPLOYEE_ID]+","+PstEmpEducation.fieldNames[PstEmpEducation.FLD_END_DATE]+ " DESC ");
        ...
        for (int i = 0; i < listEmployee.size(); i++) {
			...
			Vector listCareer = PstCareerPath.list(0, 0, PstCareerPath.fieldNames[PstCareerPath.FLD_EMPLOYEE_ID]+"="+sessTmpSpecialEmployee.getEmployeeId(), PstCareerPath.fieldNames[PstCareerPath.FLD_WORK_FROM]);
                         Vector listExpericen = PstExperience.list(0,0, PstExperience.fieldNames[PstExperience.FLD_EMPLOYEE_ID]+" = "+sessTmpSpecialEmployee.getEmployeeId(),PstExperience.fieldNames[PstExperience.FLD_END_DATE]+ " DESC ");
            Employee employee = new Employee();
            try {
                employee = PstEmployee.fetchExc(sessTmpSpecialEmployee.getEmployeeId());
            } catch (Exception exc){}
          try{
           ...
           // row = sheet.createRow((short) (i+1));
            ...
            cell = row.createCell((short) 10);cell.setCellValue(sessTmpSpecialEmployee.getPayrollGroup()!=null&& sessTmpSpecialEmployee.getPayrollGroup().length()>0?sessTmpSpecialEmployee.getPayrollGroup():"-");cell.setCellStyle(style2); 
            ...
            cell = row.createCell((short) 13);cell.setCellValue(employee.getEmailAddress()!=null&& employee.getEmailAddress().length()>0?employee.getEmailAddress():"-");cell.setCellStyle(style2); 
            cell = row.createCell((short) 14);cell.setCellValue(sessTmpSpecialEmployee.getReligion()!=null&& sessTmpSpecialEmployee.getReligion().length()>0?sessTmpSpecialEmployee.getReligion():"-");cell.setCellStyle(style2); 
            cell = row.createCell((short) 15);cell.setCellValue(PstEmployee.sexKey[sessTmpSpecialEmployee.getSexEmployee()]);cell.setCellStyle(style2); 
            //education
            // int cool = 13;
            ...
            cell = row.createCell((short) 23);cell.setCellValue("");cell.setCellStyle(style2); 

            
            
            cell = row.createCell((short) 24);cell.setCellValue(sessTmpSpecialEmployee.getMaritalStatus()+"-"+sessTmpSpecialEmployee.getMaritalNumberOfChildren());cell.setCellStyle(style2);
             cell = row.createCell((short) 25);cell.setCellValue(sessTmpSpecialEmployee.getTaxMarital()+"-"+sessTmpSpecialEmployee.getTaxNumberChildren());cell.setCellStyle(style2);
             cell = row.createCell((short) 26);cell.setCellValue(sessTmpSpecialEmployee.getBloodTypeEmployee());cell.setCellStyle(style2);
             cell = row.createCell((short) 27);cell.setCellValue(sessTmpSpecialEmployee.getAstekNumEMployee()!=null && sessTmpSpecialEmployee.getAstekNumEMployee().length()>0?sessTmpSpecialEmployee.getAstekNumEMployee():"-");cell.setCellStyle(style2);
             cell = row.createCell((short) 28);cell.setCellValue(sessTmpSpecialEmployee.getAsktekDate()!=null?Formater.formatDate(sessTmpSpecialEmployee.getAsktekDate(), "yyyy-MM-dd") :"-");cell.setCellStyle(style2);
             
            double los = 0;
            if (sessTmpSpecialEmployee.getCommercingDateEmployee()!=null){
                java.util.Date dt = sessTmpSpecialEmployee.getCommercingDateEmployee();
                if (employee.getResignedDate() != null){
                    dt = employee.getResignedDate();
                }
                long dayDiffLos = DateCalc.dayDifference(dt, new java.util.Date());
                los = EmployeeDetailPdf.getLosValue(dayDiffLos+"/365");
            }
             
             cell = row.createCell((short) 29);cell.setCellValue(sessTmpSpecialEmployee.getCommercingDateEmployee()!=null?Formater.formatDate(sessTmpSpecialEmployee.getCommercingDateEmployee(), "yyyy-MM-dd") :"-");cell.setCellStyle(style2);
             cell = row.createCell((short) 30);cell.setCellValue(los);cell.setCellStyle(style2);
             cell = row.createCell((short) 31);cell.setCellValue(sessTmpSpecialEmployee.getResignedDate()!=null?Formater.formatDate(sessTmpSpecialEmployee.getResignedDate(), "yyyy-MM-dd") :"-");cell.setCellStyle(style2);
             cell = row.createCell((short) 32);cell.setCellValue(sessTmpSpecialEmployee.getResignedReason()!=null && sessTmpSpecialEmployee.getResignedReason().length()>0? sessTmpSpecialEmployee.getResignedReason():"-");cell.setCellStyle(style2);
             cell = row.createCell((short) 33);cell.setCellValue(sessTmpSpecialEmployee.getResignedDesc()!=null && sessTmpSpecialEmployee.getResignedDesc().length()>0?sessTmpSpecialEmployee.getResignedDesc():"-");cell.setCellStyle(style2);
             cell = row.createCell((short) 34);cell.setCellValue(sessTmpSpecialEmployee.getLockerLocation()!=null && sessTmpSpecialEmployee.getLockerLocation().length()>0 && sessTmpSpecialEmployee.getLockerNumber()!=null && sessTmpSpecialEmployee.getLockerNumber().length()>0? sessTmpSpecialEmployee.getLockerLocation()+"-"+sessTmpSpecialEmployee.getLockerNumber():"-");cell.setCellStyle(style2);
             cell = row.createCell((short) 35);cell.setCellValue(sessTmpSpecialEmployee.getAddressEmployee()!=null && sessTmpSpecialEmployee.getAddressEmployee().length()>0?sessTmpSpecialEmployee.getAddressEmployee():"-");cell.setCellStyle(style2);
             
             cell = row.createCell((short) 36);cell.setCellValue(sessTmpSpecialEmployee.getAddressPermanent()!=null && sessTmpSpecialEmployee.getAddressPermanent().length()>0?sessTmpSpecialEmployee.getAddressPermanent():"-");cell.setCellStyle(style2);
             cell = row.createCell((short) 37);cell.setCellValue(sessTmpSpecialEmployee.getPhoneEmployee()!=null && sessTmpSpecialEmployee.getPhoneEmployee().length()>0?sessTmpSpecialEmployee.getPhoneEmployee():"-");cell.setCellStyle(style2);
             cell = row.createCell((short) 38);cell.setCellValue(sessTmpSpecialEmployee.getHandphone()!=null && sessTmpSpecialEmployee.getHandphone().length()>0?sessTmpSpecialEmployee.getHandphone():"-");cell.setCellStyle(style2);
            String phEmergensi="";
             ...
             cell = row.createCell((short) 39);cell.setCellValue((phEmergensi.length()>0?phEmergensi:"-")+"/"+(personEmrg.length()>0?personEmrg:"-"));cell.setCellStyle(style2);
             cell = row.createCell((short) 40);cell.setCellValue(sessTmpSpecialEmployee.getPostalCodeEmployee());cell.setCellStyle(style2);
             cell = row.createCell((short) 41);cell.setCellValue(sessTmpSpecialEmployee.getBirthPlaceEmployee()!=null && sessTmpSpecialEmployee.getBirthPlaceEmployee().length()>0? sessTmpSpecialEmployee.getBirthPlaceEmployee():"-");cell.setCellStyle(style2);
             cell = row.createCell((short) 42);cell.setCellValue(sessTmpSpecialEmployee.getBirthDateEmployee()!=null ? Formater.formatDate(sessTmpSpecialEmployee.getBirthDateEmployee(), "yyyy-MM-dd") :"-");cell.setCellStyle(style2);
             cell = row.createCell((short) 43);cell.setCellValue(sessTmpSpecialEmployee.getUmur());cell.setCellStyle(style2);
             cell = row.createCell((short) 44);cell.setCellValue( sessTmpSpecialEmployee.getRaceName()!=null && sessTmpSpecialEmployee.getRaceName().length()>0? sessTmpSpecialEmployee.getRaceName()  :"-");cell.setCellStyle(style2);
             cell = row.createCell((short) 45);cell.setCellValue( sessTmpSpecialEmployee.getFather()!=null && sessTmpSpecialEmployee.getFather().length()>0 && !sessTmpSpecialEmployee.getFather().equalsIgnoreCase("null") ? sessTmpSpecialEmployee.getFather()  :"-");cell.setCellStyle(style2);
             cell = row.createCell((short) 46);cell.setCellValue( sessTmpSpecialEmployee.getMother()!=null && sessTmpSpecialEmployee.getMother().length()>0 && !sessTmpSpecialEmployee.getMother().equalsIgnoreCase("null") ? sessTmpSpecialEmployee.getMother()  :"-");cell.setCellStyle(style2);
             cell = row.createCell((short) 47);cell.setCellValue( sessTmpSpecialEmployee.getParentsAddress()!=null && sessTmpSpecialEmployee.getParentsAddress().length()>0 && !sessTmpSpecialEmployee.getParentsAddress().equalsIgnoreCase("null") ? sessTmpSpecialEmployee.getParentsAddress()  :"-");cell.setCellStyle(style2);
            ...
            cell = row.createCell((short) 56);cell.setCellValue( "");cell.setCellStyle(style2);
            cell = row.createCell((short) 57);cell.setCellValue( "");cell.setCellStyle(style2);
            
            cell = row.createCell((short) 58);cell.setCellValue(sessTmpSpecialEmployee.getEndContractEmployee()!=null ? Formater.formatDate(sessTmpSpecialEmployee.getEndContractEmployee(), "yyyy-MM-dd") :"-");cell.setCellStyle(style2);
            cell = row.createCell((short) 59);cell.setCellValue(sessTmpSpecialEmployee.getIdCardType()!=null && sessTmpSpecialEmployee.getIdCardType().length()>0 && !sessTmpSpecialEmployee.getIdCardType().equalsIgnoreCase("null") ? sessTmpSpecialEmployee.getIdCardType()  :"-");cell.setCellStyle(style2);
            cell = row.createCell((short) 60);cell.setCellValue(sessTmpSpecialEmployee.getNpwp()!=null && sessTmpSpecialEmployee.getNpwp().length()>0 && !sessTmpSpecialEmployee.getNpwp().equalsIgnoreCase("null") ? sessTmpSpecialEmployee.getNpwp()  :"-");cell.setCellStyle(style2);
            cell = row.createCell((short) 61);cell.setCellValue(sessTmpSpecialEmployee.getNoKK()!=null && sessTmpSpecialEmployee.getNoKK().length()>0 && !sessTmpSpecialEmployee.getNoKK().equalsIgnoreCase("null") ? sessTmpSpecialEmployee.getNoKK()  :"-");cell.setCellStyle(style2);
            cell = row.createCell((short) 62);cell.setCellValue(sessTmpSpecialEmployee.getBpjsNo()!=null && sessTmpSpecialEmployee.getBpjsNo().length()>0 && !sessTmpSpecialEmployee.getBpjsNo().equalsIgnoreCase("null") ? sessTmpSpecialEmployee.getBpjsNo()  :"-");cell.setCellStyle(style2);
            cell = row.createCell((short) 63);cell.setCellValue(sessTmpSpecialEmployee.getBpjsDate()!=null ? Formater.formatDate(sessTmpSpecialEmployee.getBpjsDate(), "yyyy-MM-dd") :"-");cell.setCellStyle(style2);
            cell = row.createCell((short) 64);cell.setCellValue(sessTmpSpecialEmployee.getShio()!=null && sessTmpSpecialEmployee.getShio().length()>0 && !sessTmpSpecialEmployee.getShio().equalsIgnoreCase("null") ? sessTmpSpecialEmployee.getShio()  :"-");cell.setCellStyle(style2);
            cell = row.createCell((short) 65);cell.setCellValue(sessTmpSpecialEmployee.getElemen()!=null && sessTmpSpecialEmployee.getElemen().length()>0 && !sessTmpSpecialEmployee.getElemen().equalsIgnoreCase("null") ? sessTmpSpecialEmployee.getElemen()  :"-");cell.setCellStyle(style2);
            cell = row.createCell((short) 66);cell.setCellValue(sessTmpSpecialEmployee.getIq()!=null && sessTmpSpecialEmployee.getIq().length()>0 && !sessTmpSpecialEmployee.getIq().equalsIgnoreCase("null") ? sessTmpSpecialEmployee.getIq()  :"-");cell.setCellStyle(style2);
            cell = row.createCell((short) 67);cell.setCellValue(sessTmpSpecialEmployee.getEq()!=null && sessTmpSpecialEmployee.getEq().length()>0 && !sessTmpSpecialEmployee.getEq().equalsIgnoreCase("null") ? sessTmpSpecialEmployee.getEq()  :"-");cell.setCellStyle(style2);
            cell = row.createCell((short) 68);cell.setCellValue(sessTmpSpecialEmployee.getProbationEndDate()!=null ? Formater.formatDate(sessTmpSpecialEmployee.getProbationEndDate(), "yyyy-MM-dd") :"-");cell.setCellStyle(style2);
            cell = row.createCell((short) 69);cell.setCellValue(sessTmpSpecialEmployee.getStatusPensionProgram()!=null && sessTmpSpecialEmployee.getStatusPensionProgram().length()>0 && !sessTmpSpecialEmployee.getStatusPensionProgram().equalsIgnoreCase("null") ? sessTmpSpecialEmployee.getStatusPensionProgram()  :"-");cell.setCellStyle(style2);
            cell = row.createCell((short) 70);cell.setCellValue(sessTmpSpecialEmployee.getStartDatePensiun()!=null ? Formater.formatDate(sessTmpSpecialEmployee.getStartDatePensiun(), "yyyy-MM-dd") :"-");cell.setCellStyle(style2);
            cell = row.createCell((short) 71);cell.setCellValue(sessTmpSpecialEmployee.getPresenceCheckParameter()!=null && sessTmpSpecialEmployee.getPresenceCheckParameter().length()>0 && !sessTmpSpecialEmployee.getPresenceCheckParameter().equalsIgnoreCase("null") ? sessTmpSpecialEmployee.getPresenceCheckParameter()  :"-");cell.setCellStyle(style2);
            cell = row.createCell((short) 72);cell.setCellValue(sessTmpSpecialEmployee.getMedicalInfo()!=null && sessTmpSpecialEmployee.getMedicalInfo().length()>0 && !sessTmpSpecialEmployee.getMedicalInfo().equalsIgnoreCase("null") ? sessTmpSpecialEmployee.getMedicalInfo()  :"-");cell.setCellStyle(style2);
            
            cell = row.createCell((short) 73);cell.setCellValue(sessTmpSpecialEmployee.getWaCompany()!=null&& sessTmpSpecialEmployee.getWaCompany().length()>0?sessTmpSpecialEmployee.getWaCompany():sessTmpSpecialEmployee.getCompanyName());cell.setCellStyle(style2);   
            cell = row.createCell((short) 74);cell.setCellValue(sessTmpSpecialEmployee.getWaDivision()!=null&& sessTmpSpecialEmployee.getWaDivision().length()>0?sessTmpSpecialEmployee.getWaDivision():sessTmpSpecialEmployee.getDivision());cell.setCellStyle(style2); 
            cell = row.createCell((short) 75);cell.setCellValue(sessTmpSpecialEmployee.getWaDepartement()!=null&& sessTmpSpecialEmployee.getWaDepartement().length()>0?sessTmpSpecialEmployee.getWaDepartement():sessTmpSpecialEmployee.getDepartement());cell.setCellStyle(style2); 
            cell = row.createCell((short) 76);cell.setCellValue(sessTmpSpecialEmployee.getWaSection()!=null&& sessTmpSpecialEmployee.getWaSection().length()>0?sessTmpSpecialEmployee.getWaSection():sessTmpSpecialEmployee.getSection());cell.setCellStyle(style2); 
            cell = row.createCell((short) 77);cell.setCellValue(sessTmpSpecialEmployee.getWaPosition()!=null&& sessTmpSpecialEmployee.getWaPosition().length()>0?sessTmpSpecialEmployee.getWaPosition():sessTmpSpecialEmployee.getPosition());cell.setCellStyle(style2); 
            cell = row.createCell((short) 78);cell.setCellValue(sessTmpSpecialEmployee.getGrade()!=null&& sessTmpSpecialEmployee.getGrade().length()>0?sessTmpSpecialEmployee.getGrade():sessTmpSpecialEmployee.getGrade());cell.setCellStyle(style2); 
            ...
            cell = row.createCell((short) 86);cell.setCellValue("");cell.setCellStyle(style2);
            cell = row.createCell((short) 87);cell.setCellValue("");cell.setCellStyle(style2);
            
            // Experience add by eri 2022-04-08
            cell = row.createCell((short) 88);cell.setCellValue("");cell.setCellStyle(style2);
            cell = row.createCell((short) 89);cell.setCellValue("");cell.setCellStyle(style2);
            cell = row.createCell((short) 90);cell.setCellValue("");cell.setCellStyle(style2);
            cell = row.createCell((short) 91);cell.setCellValue("");cell.setCellStyle(style2);
                        
                        //Custom Data
            int cCustom = 91;
            for(int c=0; c<listCustom.size(); c++){
                CustomFieldMaster custom = (CustomFieldMaster)listCustom.get(c);
                String whereEmpCustom = "CUSTOM_FIELD_ID="+custom.getOID()+" AND EMPLOYEE_ID="+sessTmpSpecialEmployee.getEmployeeId();
                ...
                cell = row.createCell((short) cCustom);cell.setCellValue(valueInput);cell.setCellStyle(style2);
                cCustom++;
            }
                    ...
                    if(sessTmpSpecialEmployee.getEmployeeId()==sessEmpEducation.getEmployee_id()){
                       ...
                       if(row==null){
                            ...
                            for(int idx=0;idx<15;idx++){
                             ...
                             for(int idx=19;idx<91;idx++){
                               cell = row.createCell((short) idx);cell.setCellValue("");cell.setCellStyle(style2); 
                            }
                             for(int c=91; c<(listCustom.size()+91); c++){
                                                cell = row.createCell((short) c);cell.setCellValue("");cell.setCellStyle(style2); 
                                            }
                       }
                       
                       
       
                        cell = row.createCell((short) 16);cell.setCellValue(sessEmpEducation.getEducation()!=null?sessEmpEducation.getEducation():"-");cell.setCellStyle(style2); 
                        cell = row.createCell((short) 17);cell.setCellValue(sessEmpEducation.getGraduation()!=null?sessEmpEducation.getGraduation():"-");cell.setCellStyle(style2); 
                        cell = row.createCell((short) 18);cell.setCellValue(sessEmpEducation.getStartDate());cell.setCellStyle(style2); 
                        cell = row.createCell((short) 19);cell.setCellValue(sessEmpEducation.getEndDate());cell.setCellStyle(style2); 
                ...
                for(int idxLang=0;idxLang<listLanguage.size();idxLang++){
                    ...
                    if(sessTmpSpecialEmployee.getEmployeeId()==sessEmpLanguage.getEmployee_id()){
                       ...
                       if(row==null){
                           ...
                           for(int idx=24;idx<48;idx++){
                            cell = row.createCell((short) idx);cell.setCellValue("");cell.setCellStyle(style2); 
                           }
                           //update by satrya 2013-10-17
                           //karena ada yg pecah"
                           for(int idx=48;idx<92;idx++){
                            cell = row.createCell((short) idx);cell.setCellValue("");cell.setCellStyle(style2); 
                           }
                           for(int c=92; c<(listCustom.size()+92); c++){
                                cell = row.createCell((short) c);cell.setCellValue("");cell.setCellStyle(style2); 
                            }
                       }

                        cell = row.createCell((short) 21);cell.setCellValue(sessEmpLanguage.getLanguage()!=null?sessEmpLanguage.getLanguage():"-");cell.setCellStyle(style2); 
                        cell = row.createCell((short) 22);cell.setCellValue(sessEmpLanguage.getOral()!=null?sessEmpLanguage.getOral():"-");cell.setCellStyle(style2); 
                        cell = row.createCell((short) 23);cell.setCellValue(sessEmpLanguage.getWritten()!=null?sessEmpLanguage.getWritten():"-");cell.setCellStyle(style2); 
                        cell = row.createCell((short) 24);cell.setCellValue(sessEmpLanguage.getDescription()!=null?sessEmpLanguage.getDescription():"-");cell.setCellStyle(style2); 
                 ...
                 for(int idxFam=0; idxFam<listFamily.size();idxFam++){
                     ...
                     if(sessEmpFamilyMember.getEmployeeId()==sessTmpSpecialEmployee.getEmployeeId()){
                          ...
                          adaListFamily=true;
                            ...
                            cell = row.createCell((short) 48);cell.setCellValue( sessEmpFamilyMember.getFullName()!=null && sessEmpFamilyMember.getFullName().length()>0? sessEmpFamilyMember.getFullName()  :"-");cell.setCellStyle(style2);
                            cell = row.createCell((short) 49);cell.setCellValue( PstEmployee.sexKey[sessEmpFamilyMember.getSex()]);cell.setCellStyle(style2);
                            cell = row.createCell((short) 50);cell.setCellValue( sessEmpFamilyMember.getFamilyRelation()!=null && sessEmpFamilyMember.getFamilyRelation().length()>0? sessEmpFamilyMember.getFamilyRelation() :"-");cell.setCellStyle(style2);
                            
                            cell = row.createCell((short) 51);cell.setCellValue( sessEmpFamilyMember.getGuaranted()==0? "no" :"yes");cell.setCellStyle(style2);
                            cell = row.createCell((short) 52);cell.setCellValue( sessEmpFamilyMember.getBirthDate()!=null? Formater.formatDate(sessEmpFamilyMember.getBirthDate(), "yyyy-MM-dd") :"-");cell.setCellStyle(style2);
                            cell = row.createCell((short) 53);cell.setCellValue( sessEmpFamilyMember.getUmur());cell.setCellStyle(style2);
                            cell = row.createCell((short) 54);cell.setCellValue( sessEmpFamilyMember.getReligion()!=null && sessEmpFamilyMember.getReligion().length()>0?sessEmpFamilyMember.getReligion():"-");cell.setCellStyle(style2);
                            cell = row.createCell((short) 55);cell.setCellValue( sessEmpFamilyMember.getJob()!=null && sessEmpFamilyMember.getJob().length()>0?sessEmpFamilyMember.getJob():"-");cell.setCellStyle(style2);
                            cell = row.createCell((short) 56);cell.setCellValue( sessEmpFamilyMember.getEducation()!=null && sessEmpFamilyMember.getEducation().length()>0?sessEmpFamilyMember.getEducation():"-");cell.setCellStyle(style2);
                            cell = row.createCell((short) 57);cell.setCellValue( sessEmpFamilyMember.getAddress()!=null && sessEmpFamilyMember.getAddress().length()>0?sessEmpFamilyMember.getAddress():"-");cell.setCellStyle(style2);
                            tmpFam=tmpFam+1;
             ...
             if(listCareer!=null && listCareer.size()>0){
                 for(int idxCareer=0; idxCareer<listCareer.size();idxCareer++){
                     CareerPath  careerPath = (CareerPath)listCareer.get(idxCareer);
					  ...
					  cell = row.createCell((short) 79);cell.setCellValue( careerPath.getCompanyId()>0 ? PstCareerPath.getCompany(""+careerPath.getCompanyId())  :"-");cell.setCellStyle(style2);
					  cell = row.createCell((short) 80);cell.setCellValue( careerPath.getDivisionId()>0 ? PstCareerPath.getDivision(""+careerPath.getDivisionId())  :"-");cell.setCellStyle(style2);
					  cell = row.createCell((short) 81);cell.setCellValue( careerPath.getDepartmentId()>0 ? PstCareerPath.getDepartment(""+careerPath.getDepartmentId())  :"-");cell.setCellStyle(style2);
					  cell = row.createCell((short) 82);cell.setCellValue( careerPath.getSectionId()>0 ? PstCareerPath.getSection(""+careerPath.getSectionId())  :"-");cell.setCellStyle(style2);
					  cell = row.createCell((short) 83);cell.setCellValue( careerPath.getPositionId()>0 ? PstCareerPath.getPosition(""+careerPath.getPositionId())  :"-");cell.setCellStyle(style2);
					  cell = row.createCell((short) 84);cell.setCellValue( careerPath.getLevelId()>0 ? PstCareerPath.getLevel(""+careerPath.getLevelId())  :"-");cell.setCellStyle(style2);
					  cell = row.createCell((short) 85);cell.setCellValue( careerPath.getEmpCategoryId()>0 ? PstCareerPath.getCategory(""+careerPath.getEmpCategoryId())  :"-");cell.setCellStyle(style2);
					  cell = row.createCell((short) 86);cell.setCellValue( careerPath.getWorkFrom() != null ? Formater.formatDate(careerPath.getWorkFrom(), "yyyy-MM-dd")  :"-");cell.setCellStyle(style2);
					  cell = row.createCell((short) 87);cell.setCellValue( careerPath.getWorkTo() != null ? Formater.formatDate(careerPath.getWorkTo(), "yyyy-MM-dd")  :"-");cell.setCellStyle(style2);
...
//			  }
            ...
            // add by eri yudi 2022-04-08
          
			
            int tmpExperience=countRow;
             if(listExpericen!=null && listExpericen.size()>0){
                 for(int idxExp=0; idxExp<listExpericen.size();idxExp++){
                     Experience  empExperience = (Experience)listExpericen.get(idxExp);
						row = sheet.getRow(tmpExperience);
					  if(row==null){
						   row = sheet.createRow((short) (tmpExperience));
						   for(int idx=0;idx<79;idx++){
							  cell = row.createCell((short) idx);cell.setCellValue("");cell.setCellStyle(style2); 
							} 
                                                   for(int c=92; c<(listCustom.size()+92); c++){
					  ...
					  cell = row.createCell((short) 88);cell.setCellValue( !empExperience.getPosition().isEmpty() ? empExperience.getPosition()  :"-");cell.setCellStyle(style2);
					  cell = row.createCell((short) 89);cell.setCellValue( !empExperience.getCompanyName().isEmpty() ? empExperience.getCompanyName()  :"-");cell.setCellStyle(style2);
					  cell = row.createCell((short) 90);cell.setCellValue( empExperience.getStartDate() >0 ? ""+empExperience.getStartDate()  :"-");cell.setCellStyle(style2);
					  cell = row.createCell((short) 91);cell.setCellValue( empExperience.getEndDate( )>0 ? ""+empExperience.getEndDate()  :"-");cell.setCellStyle(style2);
					  tmpExperience=tmpExperience+1;   
                 }
             }
  
              if(tmpExperience-countRow > addRow){
                     addRow = tmpExperience - countRow;
             }
	 ...
	 for (int i=2; i < (91+listCustom.size());i++){
