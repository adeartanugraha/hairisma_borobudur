...
public class LeaveDpDetailXls extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        if (listDetailLev != null && listDetailLev.size() > 0) {
            ...
            String[] tableHeader = {
                //EMPLOYEE
                "NO", "PAYROLL", "EMPLOYEE","DEPARTMENT" ,"SECTION"
                //DP
            ...
            for (int i = 0; i < listDetailLev.size(); i++) {
                ...
                String deptName = "";
                try {
                    Department dept = PstDepartment.fetchExc(item.getDepartmentOID());
                ...
                String secName = "";
                try {
                    Section sec = PstSection.fetchExc(item.getSectionOID());
                    secName = sec.getSection();
                } catch (Exception exc){}
                cell = row.createCell((short) collPos);
