...
public class xls extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        Sheet sheet = wb.createSheet("1721_bp_A1");

        CellStyle styleStandar = wb.createCellStyle();
		...
		cell = row.createCell((short) tableHeaderItem.length+1);
        cell.setCellValue("Department");
        cell.setCellStyle(style3);
		
		cell = row.createCell((short) tableHeaderItem.length+2);
        ...
        try {
            ...
            if (listEspt != null && listEspt.size() > 0) {
                for (int i = 0; i < listEspt.size(); i++) {
					...
					try {
						employee = PstEmployee.getEmployeeByNum(espt.getEmpNum());
						
						cell = row.createCell((short) s++);
						cell.setCellValue(PstEmployee.getDepartmentName(employee.getDepartmentId()));
						cell.setCellStyle(style2);
						
						cell = row.createCell((short) s++);
						cell.setCellValue(PstEmployee.getSectionName(employee.getSectionId()));
						cell.setCellStyle(style2);
